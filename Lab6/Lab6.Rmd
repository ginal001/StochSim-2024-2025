...
```
Let's do next exercise.
```{r simulating markov chain with continuous space}
a <- 0.4
n_time <- 20
x0 <- 0.5

markov_chain_continuous <- function(x0, a, n_time){  #we dont have x0 here
  prev_x <- x0
  trajectory <- numeric(n_time)
  for (i in 1:n_time){
  if (runif(1) < a){
    x <- runif(1, min = 0, max = prev_x)
    trajectory[i] <- x
  }
  else{
    x <- runif(1, min = prev_x, max = 1)
    trajectory[i] <- x
  }
  prev_x <- x
}
...
x0 <- 0.1
{
hist(markov_chain_continuous(x0, 0.4, 2000000), probability = TRUE)
